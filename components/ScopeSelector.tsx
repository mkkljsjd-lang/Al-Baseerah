...
const ScopeSelector: React.FC<ScopeSelectorProps> = ({ onSelect, selectedScope, disabled }) => {
  const options = [
    {
      id: AnalysisScope.SARF,
      title: 'علم الصرف',
      desc: 'گردان، صیغہ اور اشتقاق',
      icon: '🔄'
    },
    {
      id: AnalysisScope.NAHW,
      title: 'علم النحو',
      desc: 'نحوی باریکیوں کا مطالعہ',
      icon: '📐'
    },
    {
      id: AnalysisScope.QURAN_HADITH,
      title: 'قرآن و حدیث',
      desc: 'معنی اور سیاق و سباق کی تحقیق',
      icon: '📖'
    }
  ];
    ...
    <div className="w-full max-w-5xl mx-auto px-2 md:px-0 mt-8">
      ...
      <div className="flex justify-end mr-4 md:mr-8">
        <div className="bg-gray-200 dark:bg-emerald-900/40 px-6 md:px-10 py-2 md:py-3 rounded-t-3xl border-t border-r border-l border-gray-300/50 dark:border-white/10 shadow-sm">
          <span className="urdu text-emerald-900 dark:text-emerald-100 font-bold text-sm md:text-lg">جامع لسانی تجزیہ</span>
        </div>
      </div>

      {/* The Folder Body Container */}
      <div className="bg-gray-100/60 dark:bg-white/5 p-4 md:p-10 rounded-b-[2.5rem] rounded-tl-[2.5rem] shadow-xl border border-gray-300/30 dark:border-white/5 relative overflow-hidden perspective-1000">
        {/* Background decorative texture for "Folder" feel */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 relative z-10">
          {options.map((opt) => (
            ...
            >
              ...
              <h3 className="font-bold text-emerald-950 dark:text-white mb-2 urdu text-2xl md:text-3xl tracking-tight relative z-10">{opt.title}</h3>
              <p className="text-sm md:text-base text-emerald-700/70 dark:text-emerald-400/70 font-medium urdu max-w-[180px] leading-relaxed relative z-10">{opt.desc}</p>
